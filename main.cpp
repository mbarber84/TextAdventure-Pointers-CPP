...
std::string monsterName[] = {"Giant", "Wardog", "Orge",
                             "Wrath", "Deamon", "Elf"};
  ...
  if (character.totalHealth >= 1 && monsterHp) {
    ...
    if (playerAttack == 1) {
      ...
      if (monsterHp >= 1) {
        ...
        if (character.totalHealth <= 0) {
          ...
          std::cout << "You Were Killed!!! \nYou reached level: "
                    << character.level << "you were obliterated by "
                    << currentMonster << "\n Game Over...." << std::endl;
          sleep(3);
          exit(0);
        }
      ...
      if (x >= 50) {
        ...
        std::cout << "You weakness has costed you " << monsterAttack + 10
                  << "You health is now " << character.totalHealth << std::endl;
        sleep(1);
        Combat();
      }
    } else {
...
void Moving() {
  ...
  if (choice == 1) {
    ...
    std::cout
        << "You fell asleep and healed, your health has increase and is now "
        << character.totalHealth << std::endl;
    sleep(1);
    HUD();
  } else if (choice == 3) {
...
void LevelUp() {
  ...
  if (character.current_xp >= character.xp_to_level) {
    ...
    std::cout
        << "Your looking strong and wiser....Congratulations you're now level "
        << character.level << std::endl;
    std::cout
